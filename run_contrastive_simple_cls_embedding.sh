...
log_dir="logs"
mkdir -p "$log_dir"
embeddings_dir="../autodl-tmp/embeddings_out/contrastive_simple_cls"
score_dir="../scores_out/contrastive_simple_cls"
mkdir -p "$score_dir"
log_file="$log_dir/contrastive_simple_cls_${timestamp}.log"

echo "===== Stage 1: 纯 CLS 输出对比学习表征 ====="
echo "Logs: $log_file"
nohup bash <<EOF > "$log_file" 2>&1 &
set -euo pipefail
echo "[Stage 1] Starting CLS-only embedding generation..."
...
echo "CLS-only experiment launched. Logs: $log_file"
