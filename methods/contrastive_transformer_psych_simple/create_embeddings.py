...
def train_encoder(
    model: SingleVAEPsychTransformer,
    device: torch.device,
    client_groups,
    batch_size: int,
    max_seq_len: int,
    ...
    for ep in range(epochs):
        ...
        metrics = {"loss": 0.0, "ctr": 0.0, "recon": 0.0, "kl": 0.0}
        ...
        for step_idx in range(steps_per_epoch):
            ...
            metrics["recon"] += float(recon_loss.detach().cpu())
            metrics["kl"] += float(kl_loss.detach().cpu())
            if enable_mask_predict:
            ...
            if (step_idx + 1) % log_interval == 0:
                ...
                logger.info(
                    "epoch %d/%d step %d/%d loss=%.4f ctr=%.4f recon=%.4f kl=%.4f",
                    ep + 1,
                    epochs,
                    step_idx + 1,
                    steps_per_epoch,
    ...
    with torch.no_grad():
        for step_idx in range(steps):
            ...
            if (step_idx + 1) % max(1, log_interval) == 0:
                ...
                logger.info(
                    "embedding gen %d/%d | %d/%d samples",
                    step_idx + 1,
                    steps,
                    processed,
                    len(relevant_client_ids),
                )
...
def main(params):
    ...
    logger.info("Training single-VAE encoder...")
    train_encoder(
        model=model,
        device=device,
