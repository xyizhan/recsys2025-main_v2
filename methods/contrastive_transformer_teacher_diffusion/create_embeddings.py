...
from methods.contrastive_transformer_teacher_diffusion.data import (
    augment_views,
    build_client_sequences,
    collate_sequences,
    load_events_df,
)
from methods.contrastive_transformer_teacher_diffusion.model import UnifiedDiffusionModel
...
def get_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--ema-momentum", type=float, default=0.995)
    p.add_argument("--cfg-drop-prob", type=float, default=0.1)
    p.add_argument("--cfg-guidance-weight", type=float, default=1.0)
...
def train_model(
    ...
    for ep in range(epochs):
        ...
        for step_idx in range(steps_per_epoch):
            ...
            if (step_idx + 1) % max(1, log_interval) == 0 or (step_idx + 1) == steps_per_epoch:
                logger.info(
                    "epoch %d/%d | step %d/%d | loss=%.4f | ctr=%.4f | diff=%.4f",
                    ep + 1,
                    epochs,
                    step_idx + 1,
                    steps_per_epoch,
                    metrics["loss"] / (step_idx + 1),
                    metrics["ctr"] / (step_idx + 1),
                    metrics["diff"] / (step_idx + 1),
                )
        logger.info(
...
def main(params):
    ...
    if 0 < params.train_client_frac < 1.0 and len(client_groups) > 0:
        ...
        if keep < len(client_groups):
            ...
            logger.info(
                "Subsampled %d/%d clients (%.1f%%) for training",
                len(train_groups),
                len(client_groups),
