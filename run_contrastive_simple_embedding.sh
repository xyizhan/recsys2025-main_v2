...
log_dir="logs"
mkdir -p "$log_dir"
embeddings_dir="../autodl-tmp/embeddings_out/contrastive_simple"
score_dir="../scores_out/contrastive_simple"
mkdir -p "$score_dir"
log_file="$log_dir/contrastive_simple_${timestamp}.log"

echo "===== Stage 1: 生成单VAE对比学习表征 ====="
echo "Logs: $log_file"
nohup bash <<EOF > "$log_file" 2>&1 &
set -euo pipefail
echo "[Stage 1] Starting simplified single-VAE embedding generation..."
...
echo "Simplified single-VAE experiment launched. Logs: $log_file"
